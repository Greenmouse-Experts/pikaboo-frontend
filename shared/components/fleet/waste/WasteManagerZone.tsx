...
import ZoneAddWasteManager from './CreateWasteManagerZone';
import WasteManagerTable from '../../admin/staff/waste/WasteManagerTable';

const FleetWasteManagerZone = () => {
    const [open, setOpen] = useState<number>(1);
    const {data, refetch, isLoading} = useGetMyUsersQuery()
  
    const waste = data?.data?.filter((where: any) => where.account_type === "Waste Manager Zone")
    ...
    <>
        <div className="p-5 lg:p-9 dash-shade rounded-lg">
          ...
          <div className="mt-5">
            ...
            {
              open === 2? <ZoneAddWasteManager refetch={refetch}/> : ""
            }
          </div>
        </div>
    </>
...
export default FleetWasteManagerZone