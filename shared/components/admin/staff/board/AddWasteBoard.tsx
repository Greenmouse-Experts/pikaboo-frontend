...
import { useLazyCreateBoardQuery } from "@/services/api/onboardSlice";
...
const AddWasteBoardForm:FC<Props> = ({refetch}) => {
  const [isBusy, setIsBusy] = useState<boolean>(false)
  const [create] = useLazyCreateBoardQuery()
  const {
    control,
    handleSubmit,
    setError,
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="flex justify-end">
          <div className="mt-8 lg:mt-16 lg:w-5/12">
            <Button title={isBusy? <PulseSpinner size={13} color='white'/> : "Create Board Proile"} disabled={!isValid} />
          </div>
        </div>
      </form>
    </>
...
export default AddWasteBoardForm;
