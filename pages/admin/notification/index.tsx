...
import { useAdminGetNotifyQuery, useGetNotifyQuery, useLazyAdminDeleteNotifyQuery, useLazyAdminReadNotifyQuery } from '@/services/api/routineSlice'
...
const NotificationPage:AppPage = () => {
  const {data:notify, isLoading, refetch} = useAdminGetNotifyQuery()
  const [read] = useLazyAdminReadNotifyQuery();
  const [deleteNo] = useLazyAdminDeleteNotifyQuery()
  ...
  const [listNo, setListNo] = useState(20)
    ...
    },
    {
      ...
      content: (
        <div>
          {unread &&
            !!unread?.length &&
            unread.slice(0, listNo).map((item: NotificationBody, index: number) => (
              <NotifyItem item={item} index={index} key={index} />
            ))}
        </div>
      ),
    ...
    <>
      <div className="">
        <div className="pt-3 dash-shade lg:mt-8 lg:w-10/12 xl:w-8/12 mx-auto bg-white p-5 lg:p-8 min-h-[400px] rounded-[20px]">
          <div>
            <Tabs tabs={tab} />
            {notify && <div className='flex justify-end'>
            <p className='text-primary underline' onClick={() => setListNo(listNo + 10)}>See More</p>
            </div>}
          </div>
          {isLoading && (
